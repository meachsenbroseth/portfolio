  ...
  <header class="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-100">
    <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-1">
        <nuxt-link v-for="link in links" :key="link.path" :to="link.path"
          ...
          <!-- underline indicator -->
          <span
            class="absolute bottom-0 left-4 right-4 h-px bg-black scale-x-0 group-hover:scale-x-100 transition-transform duration-200 origin-left"></span>
          <!-- active indicator via router-link-active -->
        </nuxt-link>
      </nav>

      <!-- Mobile hamburger -->
      <button @click="open = !open" class="md:hidden flex flex-col gap-1.5 p-2 group" aria-label="Toggle menu">
        <span class="block w-5 h-px bg-black transition-all duration-300"
          :class="open ? 'rotate-45 translate-y-2' : ''"></span>
        <span class="block w-5 h-px bg-black transition-all duration-300" :class="open ? 'opacity-0' : ''"></span>
        <span class="block w-5 h-px bg-black transition-all duration-300"
          :class="open ? '-rotate-45 -translate-y-2' : ''"></span>
      </button>
    </div>
    ...
    <div class="md:hidden overflow-hidden transition-all duration-300 ease-in-out"
      ...
      <nav class="flex flex-col px-6 py-4 gap-1">
        <nuxt-link v-for="link in links" :key="link.path" :to="link.path" @click="open = false"
